   ...
   gik = generalizedInverseKinematics('RigidBodyTree',robot,'ConstraintInputs',{'position'});
    ...
    for i = 1:length(t)
        ...
        axis([-3 3 -3 3 -3 3])
        axis vis3d;
        axes_maybe = show(robot,config);
        % axes_maybe.LineWidth = 20; % does not work
        set(gcf, 'WindowState', 'maximized') % keeps window maximized during start up to ensure highest visibility of the frames
        % Code to add red dots for the frames
        allPatches = findall(gcf, "Type", "Patch");
        for jj = 1:6
            allPatches(jj).Marker = ".";
            allPatches(jj).MarkerSize = 6;
            allPatches(jj).MarkerEdgeColor = "r";
            allPatches(jj).MarkerFaceColor = "r";
        end
